//Declaring variable to recieve input from the user when events are clicked
...
//volumeSlider= container.querySelector("myrange");
//volumeOutput=container.querySelector("volumeValue")


//This allMusic variale is an array from music-list.js file
...
//Function to load music into the allMusic array
...
function setFav(){
  let getingFavVal=document.getElementsByClassName("music-list");
  let favValue= getingFavVal.fav.setAttribute("true");
  for(let i=0; i<allMusic.length; i++){
    if(favValue=="true"){
      loadFavoriteMusic();
    }
...
function loadFavoriteMusic(){
  document.getElementById("favContent").innerHTML=favValue;

}


//volumeOutput.innerhtml = volumeSlider.value;
//volumeSlider.volumeOutput=function(){
...
volumeSlider.addEventListener("mousemove", volumeREsponse);
